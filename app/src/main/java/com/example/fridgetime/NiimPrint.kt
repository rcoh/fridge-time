...
HeatShrinkTube = 11*/
    ...
    internal class NiimbotPacket(val type: Byte, val data: ByteArray) {
        ...
        companion object {
            ...
            fun convertToByteArray(bitmap: Bitmap): ByteArray {
                ...
                val byteArray = ByteArray(width * height / 8) // Each byte represents 8 pixels (1-bit each)
                ...
                for (x in 0 until width) {
                    for (y in 0 until height) {
                        val pixel = if (bitmap.getPixel(width - x - 1, y) == Color.BLACK) 1 else 0
                        currentByte = (currentByte shl 1) or pixel
                        if ((y + 1) % 8 == 0) {
                            byteArray[index] = currentByte.toByte()
                            currentByte = 0
                            index++
                        }
                    }
                ...
                for (x in 0 until width) {
                    for (y in 0 until height) {
                        ...
                        if (Color.alpha(pixel) == 0) {
                            grayscaleBitmap.setPixel(x, y, Color.argb(1, 255, 255, 255))
                        } else {
                            grayscaleBitmap.setPixel(x, y, Color.argb(1, avg, avg, avg))
                        }
                    }
                }
    ...
    suspend fun printLabel(image: Bitmap, width: Int, height: Int, labelQty: Int = 1, labelType: Int = 1, labelDensity: Int = 2) = withContext(Dispatchers.IO) {
        ...
        val packets = NiimbotPacket.naiveEncoder(rotatedPreviewImage).toList()
        println("num packets: ${packets.size}")
        for (pkt in packets) {
            send(pkt)
        }
